...
def contactinfo(request):
    ...
    "form": Contactinfo
        ...
        seat_numbers = Seat.objects.filter(
                flight_id=request.POST['flight_id'],
                date=request.POST['date'],
                depart_time=request.POST['starttime']
            ).values_list('seat_number', flat=True)

            # Convert the queryset to a list if needed
    
        seat_numbers_list = list(seat_numbers)

            # Print or use the seat numbers as needed
        print(seat_numbers_list) 
        print("hi") 
        data['seat_numbers_list']=seat_numbers_list 
...
def payment(request):
     ...
     for seat_id in selected_seats_list:
            seat = Seat(seat_number=seat_id,flight_id= request.POST['flight_id'],date=request.POST['date'],depart_time= request.POST['starttime'])  
    ...
    if request.user.is_authenticated:
        if request.method=='POST':
            ...
            for seat_data in selected_seats:
             ...
             seat_number=seat_data['seat_number'],flight_id= request.POST['flight_id'],date=request.POST['date'],depart_time=request.POST['starttime']
        )
             seat.save()
...
def ticket_detail(request, ticket_id):
    ...
    # seat = get_object_or_404(Seat, flight_id=ticket.flight.flight_id, date=ticket.flight_ddate, depart_time=ticket.flight.starttime)
    print(ticket.flight.flight_id)
    print(ticket.flight_ddate)
    print(ticket.flight.starttime)
